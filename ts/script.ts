document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateCountdown() {
        ...
        if (distance <= 0) {
            ...
            if (secretBtn) {
                (secretBtn as HTMLButtonElement).disabled = false;
    ...
    (window as unknown as CustomWindow).scrollCarousel = function(direction: number) {
        if (carousel) {
    ...
    if (secretBtn) {
        secretBtn.addEventListener('click', function(this: HTMLButtonElement) {
            if (!this.disabled) {
                revealSecret();
            }
        });
    ...
    // Share Functions
    (window as unknown as CustomWindow).shareOnWhatsApp = function() {
    ...
    (window as unknown as CustomWindow).shareOnFacebook = function() {
        const url = window.location.href;
        window.open(`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(url)}`, '_blank');
    };
    
    (window as unknown as CustomWindow).shareOnTwitter = function() {
        const text = "Venha participar da nova viagem do Totus Tuus Marie ao Beto Carrero World em Outubro de 2025! 🙏✝️🎢";
        const url = window.location.href;
        window.open(`https://twitter.com/intent/tweet?text=${encodeURIComponent(text)}&url=${encodeURIComponent(url)}`, '_blank');
    };
    
    (window as unknown as CustomWindow).copyLink = function() {
        const url = window.location.href;
